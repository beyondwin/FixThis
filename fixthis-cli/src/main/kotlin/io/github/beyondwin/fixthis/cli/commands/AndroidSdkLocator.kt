...
internal object AndroidSdkLocator {
    ...
    ): SdkLocation? = candidates(env, userHome, projectRoot).firstOrNull { it.adb.isFile && it.adb.canExecute() }

    private fun candidates(env: Map<String, String>, userHome: File, projectRoot: File? = null): Sequence<SdkLocation> = sequence {
        sdkLocation(env["ANDROID_HOME"], "ANDROID_HOME")?.let { yield(it) }
        sdkLocation(env["ANDROID_SDK_ROOT"], "ANDROID_SDK_ROOT")?.let { yield(it) }
        projectRoot?.localPropertiesSdkDir()?.let { sdkLocation(it, "local.properties sdk.dir") }?.let { yield(it) }
