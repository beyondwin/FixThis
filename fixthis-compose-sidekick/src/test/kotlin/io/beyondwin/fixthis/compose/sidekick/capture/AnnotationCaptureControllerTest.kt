...
class AnnotationCaptureControllerTest {
    ...
    fun captureCarriesInspectionErrorsForward() {
        val annotation = controller.capture(
            AnnotationCaptureInput(
                ...
                tap = TapPoint(1f, 1f),
                nodes = emptyList(),
                errors = listOf(FixThisError("INSPECT_FAILED", "Root failed")),
    ...
    private fun appInfo(): AppInfo =
        AppInfo(packageName = "io.beyondwin.fixthis.sample", versionName = "1.0", debuggable = true)

    private fun activityInfo(): ActivityInfo =
        ActivityInfo(className = "io.beyondwin.fixthis.sample.CheckoutActivity")
