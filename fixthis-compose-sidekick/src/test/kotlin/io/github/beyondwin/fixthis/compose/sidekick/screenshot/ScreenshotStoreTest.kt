package io.github.beyondwin.fixthis.compose.sidekick.screenshot
...
class ScreenshotStoreTest {
    ...
    @Test
    fun savesFullAndCropPngUnderFixThisDateDirectory() = runBlocking {
        val bitmap = bitmap(width = 8, height = 6)

        val info = store.save(
            annotationId = "annotation-1",
            fullBitmap = bitmap,
            selectedBounds = FixThisRect(left = 2f, top = 1f, right = 6f, bottom = 5f),
        )

        assertEquals(8, info.width)
        assertEquals(6, info.height)
        assertEquals(null, info.captureFailedReason)
        assertPathEndsWith("fixthis/2026-05-04/annotation-1-full.png", info.fullPath)
    ...
    fun coercesCropBoundsInsideFullBitmap() = runBlocking {
        ...
        val info = store.save(
            ...
            fullBitmap = bitmap,
            selectedBounds = FixThisRect(left = -10f, top = 4f, right = 100f, bottom = 20f),
        )
    ...
    fun skipsCropWhenBoundsHaveNoAreaAfterCoercion() = runBlocking {
        ...
        val info = store.save(
            ...
            fullBitmap = bitmap,
            selectedBounds = FixThisRect(left = 20f, top = 20f, right = 25f, bottom = 25f),
        )
