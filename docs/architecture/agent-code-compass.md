...
3. `docs/architecture/adr/*` for durable decisions.
...
5. `docs/superpowers/*`, `docs/specs/*`, and `docs/plans/*` only as historical
   planning unless a maintained doc, ADR, or source file points to them.

## Global Boundaries

- `:fixthis-compose-core` must not depend on Android UI, MCP, CLI, Gradle
  plugin, sidekick runtime, browser DTOs, or `.fixthis/` paths.
...
- The Android app does not host MCP or HTTP. Desktop `fixthis-mcp` owns the
  console, MCP tools, session store, and queue.
- Do not commit `.fixthis/`, build outputs, generated fixture workspaces,
  screenshots, or reports unless a maintained doc says a checked-in artifact
  is required.
...
1. Read the failing assertion and the cited ADR or guide.
2. Prefer removing the dependency, moving shared helpers down, or splitting a
   hotspot file.
3. If a new dependency direction is genuinely necessary, update the ADR in
   the same change as the test rule.
4. Do not raise hotspot budgets silently. If a file shrinks, lower its budget
   in the same commit.
