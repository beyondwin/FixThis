...
| `--plugin-version` | `1.4.0` | FixThis Gradle plugin version to apply when `--apply-gradle-plugin` is set. |
...
| `--plugin-version` | `1.4.0` | FixThis Gradle plugin version to apply. |
...
```bash
fixthis version
# fixthis 1.4.0 (bridge protocol v1.3)

fixthis version --json
# {"cliVersion":"1.4.0","bridgeProtocolVersion":"1.3"}
```
