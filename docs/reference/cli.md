...
| `--plugin-version` | `0.2.1` | FixThis Gradle plugin version to apply when `--apply-gradle-plugin` is set. |
...
| `--plugin-version` | `0.2.1` | FixThis Gradle plugin version to apply. |
| `--verbose`, `-v` | off | Print the full Java stack trace on failure. |
