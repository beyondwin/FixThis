...
| `callSites` | `List<{ file, line }>?` | Present only for a candidate flagged `SHARED_COMPONENT`. A best-effort list of `{ file, line }` locations where the reused component definition is invoked (capped at 10). This is verification context, not a precise edit target; confidence stays capped at medium. Absent for single-use definitions and older payloads. |
