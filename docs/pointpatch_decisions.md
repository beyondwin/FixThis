...
- feedback session state와 `.pointpatch/feedback-sessions/` persistence는 MCP
  process가 소유
- 보안/프라이버시 부담 감소
...
새 workflow의 핵심은 `pointpatch_open_feedback_console`와 feedback queue
tools다.
...
`pointpatch_get_ui_feedback`은 기존 single-item annotation flow와의
compatibility wrapper로 유지한다.

### 범위 제한

`pointpatch_navigate_app`은 `back`, `tap`, `swipe`만 지원한다. Arbitrary
typing, scripted automation, network inspection, HTTP/WebSocket mocking,
cloud sync는 PointPatch MCP 범위가 아니다.
