...
## H5. `bootstrap-mcp.sh --write` 미검증

**상태:** Pair B Task 1 에서 `--dry-run` 만 smoke 테스트. 실제로 `~/.codex/config.toml` / `.claude/settings.json` 에 쓰는 경로는 fresh contributor 머신에서 미검증.

**왜 중요한가:** 래퍼는 thin (60 LoC) 이고 `fixthis setup --write` 자체는 기존 코드. 그러나:
- cwd 변경 후 gradle wrapper 가 정확히 잡히는지
- `--target all` 로 두 config 모두 쓰는 경로
- Codex config TOML 머지 vs 덮어쓰기 동작
- Permission 에러 (예: `~/.codex/` 가 root 소유) 처리

**검증 방법:** 새 contributor 가 첫 클론 후 1회 실행. 또는 임시:
```bash
TMP=$(mktemp -d)
cp -r . "$TMP/FixThis-test"
cd "$TMP/FixThis-test"
HOME="$TMP/fakehome" ./scripts/bootstrap-mcp.sh --package io.beyondwin.fixthis.sample
ls -la "$TMP/fakehome/.codex/" .claude/
```

**기각 조건:** 임시 HOME 으로 `--write` 가 깨끗이 떨어지면 close.
