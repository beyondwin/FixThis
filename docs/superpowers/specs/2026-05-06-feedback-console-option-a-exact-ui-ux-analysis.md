...
PointPatch 콘솔에 그대로 옮기기 위한 정밀 기준이다. 구현자는 기존
pending-item 중심 UI를 기준으로 보정하지 말고, 아래의 Option A 구조와
상태 전이를 먼저 맞춘 뒤 PointPatch 데이터/API를 연결해야 한다.
...
- PointPatch에서 density toggle을 지원하지 않는다면 단일 기본값 `13px`으로
  고정한다.
...
- current PointPatch parity: `+ New session`은 제거하고 `Save snapshot`만
...
색상 dot을 붙일지는 PointPatch 결정 사항이며 prototype CSS에는 강제 없다.

히스토리가 0개일 때의 empty state는 prototype에 정의되어 있지 않다.
PointPatch에서 `.empty` 컴포넌트 재사용 여부를 별도 결정한다.
...
density 외에 `card-minimal` 변종이 존재한다. PointPatch에서 사용하지 않더라도
인지해 둔다.
...
PointPatch 결정 사항: zoom 범위, step 값, disabled 처리(최소/최대 시 버튼
...
PointPatch에서는 `[data-w]`를 Compose semantics node hit target으로
...
- 현재 PointPatch parity 문서는 resolved pin opacity도 낮추라고 되어 있다.
...
PointPatch 구현에서는 debounce 또는 optimistic API update를 사용할 수 있지만,
...
PointPatch에서 지원하지 않는다면 "단축키 미지원"으로 명시한다.
...
PointPatch에서는 이 개념을 Compose semantics node에 매핑한다.
...
3. resolved pin opacity는 standalone에는 없고 기존 PointPatch parity 문서에는
   있다. "HTML 그대로"라면 opacity 변화 없이 두고, "상태 가독성"을 우선하면
   기존 문서처럼 opacity를 낮춘다.
4. `+ New session`은 standalone에는 있지만 기존 PointPatch parity 문서에서는
...
6. 키보드 단축키는 prototype에 없다. PointPatch에서 지원하지 않는다면
   명시적으로 "미지원"으로 고정한다.
