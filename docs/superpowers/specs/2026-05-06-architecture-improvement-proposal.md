# PointPatch 아키텍처 개선 제안서

- 작성일: 2026-05-06
- 대상: PointPatch Android 프로젝트 전체 (compose-core / compose-overlay / compose-sidekick / cli / mcp / gradle-plugin)
...
PointPatch는 모듈 경계, sealed class 기반 상태 머신, 인터페이스 기반 포트, 순수 JVM 코어 분리 등 좋은 기반이 이미 갖춰져 있다.
...
- `pointpatch-compose-overlay/src/main/kotlin/io/github/pointpatch/compose/console/studio/model/StudioModels.kt` — UI용 `Annotation`
- `pointpatch-mcp/src/main/kotlin/io/github/pointpatch/mcp/console/FeedbackConsoleItemModels.kt` — `FeedbackItem` (와이어 포맷)
...
- `pointpatch-mcp/src/main/kotlin/io/github/pointpatch/mcp/session/FeedbackSessionModels.kt`
- `pointpatch-mcp/src/main/kotlin/io/github/pointpatch/mcp/session/FeedbackSessionStore.kt`
- `pointpatch-mcp/src/main/kotlin/io/github/pointpatch/mcp/session/FeedbackSessionPersistence.kt`
- `pointpatch-mcp/src/main/kotlin/io/github/pointpatch/mcp/console/FeedbackConsolePreviewModels.kt`
- `pointpatch-compose-overlay/.../studio/history/StudioHistory.kt`
- `pointpatch-compose-overlay/.../studio/model/StudioModels.kt`
...
```
pointpatch-compose-core/src/main/kotlin/io/github/pointpatch/compose/core/
├── domain/
│   ├── annotation/
│   │   ├── Annotation.kt
│   │   ├── AnnotationStatus.kt
...
```
pointpatch-compose-overlay/.../studio/theme/
├── StudioColors.kt          ← 기존
...
`pointpatch-mcp/src/main/kotlin/io/github/pointpatch/mcp/console/FeedbackConsoleAssets.kt`는 **3,198 라인 Kotlin 파일** 안에 HTML/CSS/JS 전체를 raw string으로 박아놓았다.
...
```kotlin
// pointpatch-mcp/build.gradle.kts
...
tasks.named<JavaExec>("run") {
    systemProperty("pointpatch.console.dir", layout.buildDirectory.dir("dev/console").get().asFile.absolutePath)
...
```kotlin
private fun readResource(path: String): String {
    val devDir = System.getProperty("pointpatch.console.dir")
    if (devDir != null) {
...
`pointpatch-mcp/.../session/FeedbackSessionService.kt`가 다음을 모두 담당한다:
...
| 파일 | 현재 | 권장 분해 |
|---|---|---|
| `PointPatchOverlayController.kt` (317) | 상태 머신 + 부수효과 | `OverlayStateMachine`(순수) + `OverlayEffectExecutor`(I/O) |
...
| `PointPatchTools.kt` (797) | 모든 MCP tool 한 파일 | tool 카테고리별 분리 (`SessionTools`, `AnnotationTools`, `SnapshotTools`) |
...
dependencyAnalysis {
    issues {
        ...
        project(":pointpatch-compose-core") {
