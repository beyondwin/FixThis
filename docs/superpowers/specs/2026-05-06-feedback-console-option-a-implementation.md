...
`fixthis-compose-core` 모듈의 `io.beyondwin.fixthis.compose.console.studio`
...
모든 모델은 `io.beyondwin.fixthis.compose.console.studio.model` 패키지에 둔다.
...
hex 토큰이 스펙에 명시되어 있다. `io.beyondwin.fixthis.compose.console.studio.theme`
...
전체 트리는 다음과 같다. 모든 함수는 `io.beyondwin.fixthis.compose.console.studio`
패키지에 둔다.
