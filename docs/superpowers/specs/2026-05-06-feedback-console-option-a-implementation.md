...
교체 대상: `fixthis-mcp/src/main/kotlin/io/github/fixthis/mcp/console/FeedbackConsoleAssets.kt`
...
`fixthis-compose-core` 모듈의 `io.github.beyondwin.fixthis.compose.console.studio`
...
모든 모델은 `io.github.beyondwin.fixthis.compose.console.studio.model` 패키지에 둔다.
...
hex 토큰이 스펙에 명시되어 있다. `io.github.beyondwin.fixthis.compose.console.studio.theme`
...
`Inter` 폰트는 `fixthis-compose-core/src/main/res/font` (또는 동급)에
...
전체 트리는 다음과 같다. 모든 함수는 `io.github.beyondwin.fixthis.compose.console.studio`
패키지에 둔다.
...
`DevicePreviewContent`는 본 가이드 범위 밖이며, 실제 FixThis live
...
`fixthis-mcp/src/main/kotlin/io/github/fixthis/mcp/console/FeedbackConsoleAssets.kt`는
HTML/CSS/JS 자산이고, 본 가이드의 Compose 구현은 별도 모듈 (`fixthis-compose-core`)에
...
- 본 가이드의 `StudioShell`은 `fixthis-compose-core`의 새 화면이며,
...
`fixthis-compose-core/src/main/kotlin/io/github/fixthis/compose/console/studio/`
하위에 다음 파일을 만든다.
...
`DevicePreviewContent`는 본 문서의 placeholder다. 실제 FixThis는
`/api/preview/{id}/screenshot/full`을 받아 비트맵으로 렌더한다. Compose
in-app에서는 동일 API를 호출하거나, 디바이스 직접 캡처를 거친다 — 그
부분은 `fixthis-compose-overlay` 또는 `fixthis-compose-sidekick`의
...
- 새 Compose UI 테스트는 `fixthis-compose-core/src/test/kotlin/.../studio/`에
  추가한다:
...
스펙은 1100px+ 기준 desktop 레이아웃이며, 폰 폭에서는 의미가 없다. FixThis
