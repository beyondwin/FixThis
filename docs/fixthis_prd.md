# FixThis for Android Compose — 상세 PRD
...
> Current implementation note: this PRD includes early product exploration.
> Current mainline behavior is MCP feedback console first, local session
> persistence, and Android app status-only UI. Historical in-app annotation,
> floating button, and clipboard-first sections are background unless explicitly
> marked as current.
> Current console labels and semantics are documented in
> [`docs/feedback-console-contract.md`](feedback-console-contract.md).

---

## 1. 제품 개요

FixThis for Android Compose는 실행 중인 Android Jetpack Compose debug 앱에서 사용자가 UI 요소를 직접 선택하고, 원하는 변경사항을 입력하면, 해당 UI에 대한 runtime context를 AI 코딩 에이전트에게 전달하는 개발용 도구다.

사용자는 “이 버튼”, “이 카드”, “세 번째 리스트 아이템”, “여기 여백”처럼 애매한 UI 수정 요청을 앱 화면에서 직접 지목할 수 있다. FixThis는 선택된 UI의 Compose semantics, bounds, 주변 텍스트, screenshot crop, source search hint, source candidate를 수집해 Codex, Claude Code, Cursor 같은 AI coding agent가 이해할 수 있는 Markdown/JSON context로 변환한다.

현재 mainline 제품은 MCP feedback console을 기본 agent workflow로 제공한다.

1. **현재 기본 UX:** MCP feedback console → desktop browser에서 device 선택,
   live preview로 앱 탐색, `Annotate`로 preview freeze, `Add annotation`으로
   pending feedback 작성, `Copy Prompt` 또는 `Send Agent`가 필요 시 evidence
   snapshot과 item을 저장하고 agent-readable handoff context를 생성.
2. **Historical UX:** 앱 안에서 UI 선택 → 코멘트 입력 → Copy for AI. 이 흐름은 초기 PRD 배경으로 남아 있지만 현재 mainline 구현 표면은 아니다.

MCP는 AI coding agent와의 연결을 자동화하는 primary integration layer다. Android 앱 내부에는 MCP browser 연결 상태만 표시한다.
...
FixThis는 실행 중인 앱에서 선택한 UI 요소를 기준으로 다음 정보를 제공한다.
...
FixThis는 기본적으로 Compose semantics tree를 사용한다. `testTag`는 있으면 활용하지만 필수는 아니다.
...
사용자는 Activity마다 install을 호출하거나, `setContent`를 `FixThisRoot`로 감싸거나, AndroidManifest를 직접 수정하지 않아도 된다.

### 2.4 Debug-only, privacy-first

FixThis는 개발과 디버그 용도다. production feature가 아니다.
...
→ Annotate로 preview freeze
→ target 선택과 comment 입력
→ Add annotation
→ Copy Prompt 또는 Send Agent
→ AI가 코드 수정
```
...
DTA, mobile-mcp, appium-mcp 같은 도구는 모바일 자동화, 테스트, 디버깅, network inspection에 강하다. FixThis는 더 좁고 명확한 문제에 집중한다.
...
8. MCP feedback console에서 사용자가 Studio UI의 Sessions / preview canvas /
   Inspector 구조 안에서 live preview를 탐색하고, `Annotate`로 freeze한 화면에
   component 또는 custom area feedback을 여러 개 쌓은 뒤, `Add annotation`으로
   pending annotation을 만들고 `Copy Prompt` 또는 `Send Agent`로 handoff context를
   만들 수 있다.
...
```text
FixThis lets you point at Jetpack Compose UI in a debug Android app, describe what you want changed, and send precise runtime context to AI coding agents.
```

한국어:

```text
FixThis는 Android Jetpack Compose debug 앱에서 수정할 UI를 지목하고 원하는 변경사항을 입력하면, AI 코딩 에이전트가 코드를 수정할 수 있는 정확한 runtime context를 생성합니다.
```
...
- **MCP console first:** 현재 mainline UX는 desktop MCP feedback console을 기본 agent workflow로 둔다.

---

## 7. 핵심 사용자 시나리오

### 7.1 기본 시나리오: MCP console에서 선택하고 저장

현재 기본 시나리오는 desktop browser console이다.
...
### 7.1.1 Historical 시나리오: 앱에서 선택하고 복사

```text
1. 사용자가 debug 앱을 실행한다.
2. 화면 우하단 FixThis 버튼이 보인다.
3. 사용자가 FixThis 버튼을 누른다.
...
5. 피드백을 남길 화면에서 `Annotate`를 눌러 최신 preview를 freeze한다.

6. component를 클릭하거나 custom area를 drag하고 comment를 입력한 뒤
   `Add annotation`을 누른다. 같은 frozen preview에 여러 pending item을 추가할 수
   있고, pending item은 Focus/Delete만 지원한다.

7. `Copy Prompt` 또는 `Send Agent`는 필요 시 frozen preview를 evidence snapshot
   하나로 저장하고 모든 pending item을 같은 `screenId`에 연결한다.

8. `Send Agent`를 누르면 console은 외부 AI API를 호출하지 않고,
...
3. FixThis는 선택 좌표, 후보 nodes, screenshot crop, 주변 nodes를 export한다.
...
이 방식은 source index가 없을 수 있지만, runtime inspection, screenshot capture, MCP bridge status는 동작해야 한다.

### 8.2 앱 안 상태 UX

현재 앱 화면에는 feedback selection floating button이 아니라 MCP browser
connection status pill만 표시된다.
...
선택, 코멘트, `Copy Prompt`, `Send Agent`는 desktop feedback console에서 처리한다.

Status pill은 최근 인증된 MCP browser heartbeat를 기준으로 연결 상태만 보여준다.

### 8.3 Feedback console selection UX

`Annotate`를 누르면 최신 preview가 freeze되고 browser console이 feedback selection mode로 들어간다.

```text
- live preview polling pause
- frozen screenshot and semantics tree 유지
- semantics node 클릭 또는 visual area drag
- numbered overlay marker 표시
- comment 입력 후 `Add annotation`
```

### 8.4 Pending and saved feedback UX

Pending item은 `Copy Prompt` 또는 `Send Agent`로 저장되기 전 browser state에만 있고 다음 UI를 제공한다.

- comment
- selected target summary
- numbered preview marker
- Focus
- Delete

`Copy Prompt` 또는 `Send Agent`는 필요 시 frozen preview 하나를 persisted evidence
snapshot 하나로 승격하고, 그 preview에서 만든 pending item들을 같은 `screenId`에
연결한다. Saved evidence group은 persisted screenshot, numbered overlay, comments,
optional `targetEvidence`를 보여준다.
...
현재 repo의 sample app은 Gradle project `:app`으로 노출되고 source는 `sample/` 아래에 둔다. 따라서 local smoke flow와 `fixthis run`의 기본 install task는 `:app:installDebug`다.
...
```text
- --package 값 또는 .fixthis/project.json의 package/project metadata 확인
- fixthis mcp command와 args를 포함한 JSON config 출력
```
...
- Studio UI에서 live preview navigation, `Annotate` freeze, `Add annotation`,
  `Copy Prompt`/`Send Agent` handoff workflow 제공
```
...
`fixthis-compose-sidekick`은 debug APK 안에 포함되는 Android runtime이다.
...
현재 tool surface는 feedback console session queue를 중심으로 한다.

1. `fixthis_status`
2. `fixthis_get_current_screen`
3. `fixthis_verify_ui_change`
4. `fixthis_open_feedback_console`
5. `fixthis_list_feedback_sessions`
6. `fixthis_capture_screen`
7. `fixthis_navigate_app`
8. `fixthis_list_feedback`
9. `fixthis_read_feedback`
10. `fixthis_resolve_feedback`

Historical note: earlier PRD revisions included `fixthis_get_ui_feedback` as a
single-feedback compatibility wrapper. Current mainline feedback capture goes
through the console `Annotate` / `Add annotation` / `Copy Prompt` / `Send Agent`
flow instead.
...
→ Annotate로 최신 preview freeze
→ component click 또는 custom drag area 선택
→ comment 입력 후 Add annotation으로 여러 pending item 추가
→ Copy Prompt로 compact Markdown 복사 또는 Send Agent로 persisted handoff batch 생성
→ saved evidence group과 fixthis_read_feedback에서 batch 확인
```
...
{
  "schemaVersion": "1.0",
  "id": "annotation-20260503-143012-8f4b",
  ...
  "screenshot": {
    "fullPath": "...",
    "cropPath": "...",
    "desktopFullPath": ".fixthis/artifacts/annotation-20260503-143012-8f4b/annotation-20260503-143012-8f4b-full.png",
    "desktopCropPath": ".fixthis/artifacts/annotation-20260503-143012-8f4b/annotation-20260503-143012-8f4b-crop.png"
  },
...
- full: `/data/user/0/.../fixthis/...-full.png`
- crop: `/data/user/0/.../fixthis/...-crop.png`
```
...
- `fixthis setup`
- `--package` 또는 `.fixthis/project.json` 기반 setup JSON 출력
...
- desktop feedback console에서 active device를 선택하고 live preview를 볼 수 있다.
- `Select` mode에서 preview click은 debug app navigation으로 동작한다.
- `Annotate`를 누르면 selection layer가 표시된다.
- Button/Text/TextField/Switch/Checkbox/LazyColumn item을 선택할 수 있다.
- 선택 또는 custom area drag 후 browser inspector에서 comment를 입력할 수 있다.
- `Add annotation`으로 pending annotation을 만들 수 있다.
- `Copy Prompt`가 compact Markdown을 clipboard에 복사한다.
- `Send Agent`가 local handoff batch를 저장한다.
...
- `fixthis status`가 sidekick 연결 상태를 표시한다.
- `fixthis setup --package <applicationId>` 또는 기존 `.fixthis/project.json` 기반 setup이 MCP config를 출력한다.
- `fixthis_open_feedback_console`이 browser feedback console URL을 반환한다.
- feedback console 기본 상태는 `Select`이며 `Annotate`로 최신 preview를 freeze할 수 있다.
- 한 frozen preview에서 component click 또는 custom drag area로 pending item 여러 개를 만들 수 있다.
- `Copy Prompt` 또는 `Send Agent`가 필요 시 evidence snapshot 1개와 item N개를 저장하고 같은 `screenId`를 공유한다.
- `Send Agent` 이후 `fixthis_read_feedback`이 compact Markdown과 complete JSON handoff batch를 반환한다.
- `fixthis_verify_ui_change`가 expected text 존재 여부를 검사한다.
...
FixThis for Android Compose는 “모바일 자동화 MCP”가 아니라 “Compose UI 수정 요청용 runtime context 도구”다.
...
현재 구현은 MCP feedback console을 기본 agent workflow로 둔다. 앱 안에는 피드백 선택 UI를 두지 않고 MCP browser 연결 상태만 표시한다.
...
- no AccessibilityService
- one-line install
- browser-console UI selection
- browser-console comment input
- screenshot crop
- semantics context
- source candidates
- clipboard/export
- primary MCP feedback console
- easy CLI setup
```
