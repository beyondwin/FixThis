...
→ comment 입력
→ Save로 evidence snapshot과 feedback item 저장
→ Copy 또는 Send
```
...
선택, 코멘트, Copy, Send는 desktop feedback console에서 처리한다.

Status pill은 최근 인증된 MCP browser heartbeat를 기준으로 연결 상태만 보여준다.

### 8.3 Feedback console selection UX

Add를 누르면 최신 preview가 freeze되고 browser console이 feedback selection mode로 들어간다.
...
- comment 입력 후 Add to Pending
```

### 8.4 Pending and saved feedback UX

Pending item은 Save 전 browser state에만 있고 다음 UI를 제공한다.
...
Save는 frozen preview 하나를 persisted evidence snapshot 하나로 승격하고,
그 preview에서 만든 pending item들을 같은 `screenId`에 연결한다. Saved evidence
group은 persisted screenshot, numbered overlay, comments, optional
`targetEvidence`를 보여준다.
...
through the console Add/Save/Send flow instead.
...
- debug app에서 floating FixThis button이 보인다.
...
- Send 이후 `fixthis_read_feedback`이 compact Markdown과 complete JSON handoff batch를 반환한다.
