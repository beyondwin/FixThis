...
신규 유저가 PointPatch를 Claude Code 또는 Codex에 연결하려면 다음을 직접 수행해야 한다.

```
1. 레포 클론
2. ./gradlew :pointpatch-cli:installDist :pointpatch-mcp:installDist
...
`pointpatch setup` 명령은 JSON을 stdout에 출력하기만 하고, 실제 파일에 쓰지 않는다. 유저가 올바른 파일 위치를 찾아 형식에 맞게 직접 작성해야 한다.
...
- `pointpatch setup --write` 한 명령으로 Claude Code / Codex 양쪽 설정 완료
...
- Windows 지원 — V1 범위 외
- 자동 PATH 등록 — 별도 `pointpatch install` 커맨드로 분리
...
```
pointpatch setup [--package <id>] [--project-dir <path>]
                 [--write]                     ← 신규: 파일에 자동 기록
...
pointpatch    /절대경로/pointpatch-cli/build/install/pointpatch/bin/pointpatch
...
```kotlin
// 신규 파일: pointpatch-cli/.../AndroidSdkLocator.kt
...
merge 로직:
1. 파일이 없으면 새로 생성
2. 있으면 `mcpServers.pointpatch` 키만 교체, 나머지 보존
...
```toml
[mcp_servers.pointpatch]
command = "/절대경로/pointpatch"
args = ["mcp", "--package", "...", "--project-dir", "..."]

[mcp_servers.pointpatch.env]
ANDROID_HOME = "/절대경로/sdk"
```
...
merge 로직:
1. 기존 파일을 줄 단위로 읽음
2. `[mcp_servers.pointpatch]` ~ 다음 `[` 섹션 사이를 찾아 교체
3. 없으면 파일 끝에 append
4. 다른 섹션은 그대로 보존
...
data class McpConfigEntry(
    val serverName: String,          // "pointpatch"
    val command: String,             // 절대경로 or "pointpatch"
...
class SetupCommand : CoreCliktCommand(name = "setup") {
    ...
    override fun run() {
        ...
        val entry = McpConfigEntry(
            serverName = "pointpatch",
            command = binaryPath?.absolutePath ?: "pointpatch",
        ...
        if (!write) {
            // 기존 동작: JSON stdout
            echo(pointPatchJson.encodeToString(buildMcpClientConfig(resolvedPackage, root)))
            return
        }
...
- PointPatch CLI는 `adb.devices()`로 연결된 디바이스를 자동 감지한다
- 여러 디바이스가 연결된 경우에는 `doctor` 명령이 안내

단, `--serial` 옵션을 명시적으로 전달하면 env에 포함:

```
pointpatch setup --write --serial adb-R3CN60LXW3L-...
```
...
| 파일 | 변경 유형 | 내용 |
|------|----------|------|
| `pointpatch-cli/.../commands/SetupCommand.kt` | 수정 | `--write`, `--target`, `--dry-run` 옵션 추가 |
| `pointpatch-cli/.../AgentConfigWriter.kt` | 신규 | Writer 인터페이스 + `McpConfigEntry` 모델 |
| `pointpatch-cli/.../AndroidSdkLocator.kt` | 신규 | ANDROID_HOME 자동 탐색 |
| `pointpatch-cli/.../ClaudeConfigWriter.kt` | 신규 | `.claude/settings.json` merge 구현 |
| `pointpatch-cli/.../CodexConfigWriter.kt` | 신규 | `~/.codex/config.toml` merge 구현 |
| `pointpatch-cli/.../commands/SetupCommandTest.kt` | 신규 | Writer 선택, 옵션 파싱 테스트 |
| `pointpatch-cli/.../ClaudeConfigWriterTest.kt` | 신규 | merge 로직 단위 테스트 |
| `pointpatch-cli/.../CodexConfigWriterTest.kt` | 신규 | TOML 섹션 교체 단위 테스트 |
...
// 2. 기존 다른 mcpServer가 있을 때 → pointpatch만 교체, 나머지 보존
merge("""{"mcpServers":{"playwright":...}}""", entry) shouldContain "playwright"

// 3. 기존 pointpatch가 있을 때 → 덮어씀
merge("""{"mcpServers":{"pointpatch":{"command":"old"}}}""", entry) shouldContain newCommand
...
```kotlin
// 1. 빈 파일 → 섹션 append
merge(null, entry) shouldContain "[mcp_servers.pointpatch]"

// 2. 기존 다른 mcp_server 존재 → 보존
merge("[mcp_servers.playwright]\ncommand = \"npx\"", entry) shouldContain "playwright"

// 3. 기존 pointpatch 섹션 → 교체
merge("[mcp_servers.pointpatch]\ncommand = \"old\"", entry) shouldNotContain "old"

// 4. pointpatch가 마지막 섹션이 아닐 때 → 다음 섹션까지만 교체
```
...
9. `pointpatch install` 커맨드 — `~/.local/bin` 에 심볼릭 링크
...
2. **기존 pointpatch 설정 감지**: `--write` 실행 시 이미 설정이 존재하면 overwrite 전에 확인 메시지를 보여줄 것인가, 조용히 덮어쓸 것인가? 현재 설계는 조용히 merge (덮어씀).

3. **빌드되지 않은 경우**: `McpExecutableLocator.find()`가 null을 반환하면 `command = "pointpatch"`를 넣는다. 이 경우 PATH에 `pointpatch`가 없으면 MCP 클라이언트 실행 시 실패한다. 경고 메시지 출력 필요.
