...
> 현재 코드 상태: `io.beyondwin.fixthis.*` 패키지로 리네임 완료, Compose BOM `2026.04.01`, Kotlin `2.2.21`, AGP `9.1.1`.
...
- `targetEvidence`를 새 최상위 모델로 만들지 말고, **기존 `FixThisAnnotation`에 `targetEvidence: TargetEvidence?` 필드를 옵셔널로 추가**한다. 스키마 버전을 `1.1`로 올리고 디폴트 값으로 채워 하위 호환을 유지한다. `AnnotationDto`에도 동일한 필드를 매핑해 `SessionDomainMappers`에서 통과시킨다.
...
1. **컨벤션의 적용 범위가 정의되어 있지 않다.** 사용자가 `comp:` 접두를 안 쓰면 그냥 평문 testTag로 처리한다는 건 합리적인데, 그렇다면 *왜 sample app부터 이 컨벤션을 적용하지 않는가*라는 질문이 따라온다. 현재 `sample/`의 `StudioTestTags.kt`나 화면별 컴포넌트들은 모두 평문 testTag를 쓴다. 플랜이 가치를 보이려면 sample을 통한 골든 패스 시연이 필요하다. 즉 *플랜이 실제로 검증되려면 sample 리팩터가 동반되어야 한다*는 작업 의존성을 명시해야 한다.
...
- sample 앱의 `StudioTestTags`를 `comp:Studio*` 컨벤션으로 마이그레이션하는 작업을 *Phase 1의 명시적 완료 기준*에 포함시켜야 데모가 가능하다.
...
4. **MERGED vs UNMERGED**: 현재 `inspectCurrentScreen`은 두 트리를 모두 반환한다. occurrence를 어느 트리 기준으로 셀지 정해야 한다. **MERGED 트리 기준 + (선택된 노드가 unmerged면 그 merged 부모로 환원)**이 직관적이다. 부모 환원 로직은 `FixThisNode.path`에 ancestor uid가 있으니 가능하다.
...
1. **현재 데이터 모델로 subtree 추출이 어렵다.** `FixThisNode.path`는 *조상* uid 리스트만 가지고 있다. 자식 관계는 직접 저장되어 있지 않다. `inspectCurrentScreen`은 노드를 평탄화한 리스트로 반환한다.
...
- `FixThisNode`에 `parentUid: String?` 추가(스키마 1.1).
- compose-core `format/SubtreeRenderer.kt` 신설 — 평탄화된 노드 리스트 + selectedUid를 받아 트리를 재구성하고 cutoff 적용해 String으로 렌더한다. 디바이스 의존이 없으므로 `compose-core`(JVM 모듈)에 두는 것이 적절하다.
- merged 트리 기준이 디폴트, full 모드일 때만 unmerged subtree도 함께 출력.
...
- **`CompositionData.mapTree` / `parseSourceInformation`**: `androidx.compose.ui.tooling.data` 패키지 함수. **현재 프로젝트는 `compose-ui-tooling-data` 의존성을 *명시적으로 추가하지 않았다***. `libs.versions.toml`을 확인하면 `compose-ui-tooling`(디버그)과 `compose-ui-tooling-preview`만 있다. `tooling-data`를 추가해야 하는데, 이는 BOM에 매핑되지 않고 별도 버전 선언이 필요할 수 있다(BOM에 포함된 시점은 1.4 이후). 의존성 추가 비용은 작지만, *runtime*에서만 필요한 게 아니라 inspectable 트리 활성화는 컴포지션 단계의 런타임 효과이므로 *프로덕션 빌드에서도 항상 잡혀야 한다는 부담*이 있다.
...
- `compose-ui-tooling-data` 의존성을 명시적으로 추가하고, 그 버전을 BOM과 분리해 관리한다(매 BOM 업데이트 시 회귀 가능성을 별도로 감시).
...
- `SubtreeRenderer`: cutoff(maxDepth/maxNodesPerLevel) 동작, 사이클 방지(parentUid가 자기 자신을 가리키는 깨진 데이터), redacted 노드의 표시(`<redacted>` 등 — `RedactionPolicy`가 이미 적용됨).
...
- `FixThisAnnotation`에 새 필드(`targetEvidence`, `parentUid` 등)가 추가되면 `kotlinx.serialization`이 자동으로 default를 받지만, **JSON 라운드트립 골든 테스트가 있다면 깨진다**. `SourceMatcherTest`, `NodeSelectorTest`, `FixThisMarkdownFormatterTest`, `FeedbackQueueFormatterTest`가 영향권. 사전 점검 필수.
...
```
[A] 데이터 모델 변경
    A1. FixThisNode.parentUid 추가 (스키마 1.1)
...
[B] 순수 함수 / 디바이스 비의존 로직
    ...
    B4. SubtreeRenderer (compose-core)
    B5. DetailMode enum + FixThisMarkdownFormatter(detail) 분기

[C] 디바이스 사이드 통합
    C1. SemanticsNodeMapper에서 parentUid 채우기
...
[F] Phase 2 (실험)
    F1. compose-ui-tooling-data 의존성 추가
...
- **B는 A에 비해 거의 병렬 가능**(parentUid를 안 쓰는 함수는 즉시 가능)
...
| 가정 | 검증 |
|---|---|
| `comp:AppPrimaryButton:primary` 같은 testTag가 sample에 존재 | sample은 현재 `StudioTestTags` 형태의 평문이다. **선결 작업 필요**. |
...
2. "subtree 출력이 가능하다" → `parentUid`가 필요.
...
Compose 1.8.x에서 `LocalInspectionTables`의 정확한 위치/타입은 1.5/1.6 시점과 다르다. `tooling-data` 의존성 버전을 BOM과 별도로 고정하면 컴파일 시점의 API mismatch가 생길 수 있다. *런타임 ClassNotFoundException* 가능성도 있다. Phase 2 도입 전에 sample 앱에서 `tooling-data` 단독 호출 한 줄이 컴파일되는지부터 검증해야 한다.

### 7.4 protocol versioning

`BridgeProtocol.VERSION`이 존재한다(상태 응답에 노출). evidence schema가 1.1로 올라가면 **BridgeProtocol.VERSION도 같이 올려서**, 구버전 sidekick과 신버전 MCP가 섞일 때 조기 실패하도록 한다. 현재 코드에는 protocol negotiation이 없다.
...
| 3 | `FixThisNode.parentUid` 추가 | subtree 렌더링의 전제 |
...
| 12 | `BridgeProtocol.VERSION`을 schema 1.1과 함께 올림 | 혼합 버전 안전 |
...
- `fixthis-compose-core/src/main/kotlin/io/beyondwin/fixthis/compose/core/model/Models.kt` — 스키마 추가
- `fixthis-compose-core/src/main/kotlin/io/beyondwin/fixthis/compose/core/format/FixThisMarkdownFormatter.kt` — DetailMode
- `fixthis-compose-core/src/main/kotlin/io/beyondwin/fixthis/compose/core/source/SourceMatcher.kt` — 컨벤션 가산
- `fixthis-compose-core/src/main/kotlin/io/beyondwin/fixthis/compose/core/identity/` (신설) — TestTagConvention, IdentityHint, OccurrenceCalculator, SubtreeRenderer
- `fixthis-compose-sidekick/src/main/kotlin/io/beyondwin/fixthis/compose/sidekick/inspect/SemanticsNodeMapper.kt` — parentUid 채우기
- `fixthis-compose-sidekick/src/main/kotlin/io/beyondwin/fixthis/compose/sidekick/screenshot/{ScreenshotCapturer,ScreenshotStore}.kt` — highlight/paddedCrop
- `fixthis-compose-sidekick/src/main/kotlin/io/beyondwin/fixthis/compose/sidekick/bridge/BridgeServer.kt` — readScreenshot kind 확장, BridgeProtocol.VERSION
- `fixthis-mcp/src/main/kotlin/io/beyondwin/fixthis/mcp/tools/FixThisTools.kt` — fixthis_read_feedback에 detailMode
- `fixthis-mcp/src/main/kotlin/io/beyondwin/fixthis/mcp/session/FeedbackQueueFormatter.kt` — DetailMode 분기
- `fixthis-mcp/src/main/kotlin/io/beyondwin/fixthis/mcp/session/{SessionDtoModels,SessionDomainMappers,FeedbackHandoffModels}.kt` — DTO 매핑
- `fixthis-mcp/src/main/kotlin/io/beyondwin/fixthis/mcp/console/FeedbackConsoleAssets.kt` — 토글 UI
- `fixthis-gradle-plugin/src/main/kotlin/.../task/GenerateFixThisSourceIndexTask.kt` — 컨벤션 분해 인덱싱(권고 6 연계)
- `sample/src/main/java/io/beyondwin/fixthis/sample/**` — testTag 마이그레이션
- `gradle/libs.versions.toml` — `compose-ui-tooling-data` 추가(Phase 2)
