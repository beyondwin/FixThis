# FixThis for Android Compose — 주요 결정사항 및 근거

**문서 목적:** 이 문서는 FixThis for Android Compose의 주요 제품/기술 의사결정을 기록한다. 각 결정이 왜 내려졌는지, 어떤 대안을 검토했는지, 무엇을 포기했는지를 명확히 남긴다.
**문서 버전:** v1.0  
**작성일:** 2026-05-03

> Current implementation note: this decision log preserves early product
> decisions even where the shipped mainline has moved on. Current operational
> docs should treat MCP feedback console first, Android app status-only UI, and
> nullable Stable Target Evidence as the source of truth.

---

## 1. 최종 방향 요약

FixThis는 Android Jetpack Compose debug 앱에서 사용자가 수정할 UI를 직접 지목하고, 원하는 변경사항을 입력하면, AI 코딩 에이전트에게 정확한 runtime context를 전달하는 도구다.
...
## 2. 결정 1: 이름을 “FixThis for Android Compose”로 정한다
...
FixThis는 사용자 행동과 제품 결과를 모두 담는다.
...
FixThis는 Android Jetpack Compose만 지원한다.
...
FixThis는 debug/dev utility다. production feature가 아니다.

### 이유

FixThis는 화면 텍스트, screenshot, UI tree, source hints를 다룬다. 이는 민감한 정보를 포함할 수 있다. 따라서 production 앱에 들어가거나 일반 사용자에게 노출되어서는 안 된다.
...
FixThis는 tag 없는 기본 Compose UI에서도 동작해야 한다.
...
AccessibilityService는 앱 코드 수정 없이 UI tree를 얻을 수 있는 강력한 방법이다. 하지만 FixThis의 목적에는 과하다.
...
FixThis는 앱 내부 debug sidekick으로 동작한다. 타 앱 inspector가 아니다.
...
FixThis는 Compose `SemanticsOwner`의 merged/unmerged semantics tree를 읽어 UI node context를 만든다.
...
FixThis는 merged semantics tree와 unmerged semantics tree를 모두 수집한다.
...
## 10. 결정 9: `FixThisRoot {}` wrapper는 기본 UX에서 제외한다

### 결정

`FixThisRoot` 같은 wrapper는 fallback API로만 제공한다. 기본 UX는 autoinit이다.
...
artifact가 publish된 뒤의 외부 프로젝트 UX는 다음이어야 한다.
...
`fixthis-compose-sidekick`은 AndroidX Startup initializer를 포함한다.

### 이유

사용자가 앱 코드에 `FixThis.install()`을 직접 넣지 않아도 되게 하려면 auto-init이 필요하다.
...
Historical decision: 초기 설계에서는 기본 UX를 MCP가 아니라 앱 안 annotation으로 두었다.
...
Current mainline correction: shipped V1은 MCP feedback console을 기본 agent
workflow로 두며, Android 앱 내부에는 `MCP waiting`/`MCP connected` 상태만 표시한다.
...
이 결정은 제품 탐색 단계의 근거로 보존한다. 현재 구현 기준의 운영 문서는
MCP console Add/Save/Send 흐름을 우선한다.
...
Historical decision: MCP는 제공하되, 기본 UX가 아니라고 보았다.
...
```text
Current:
MCP feedback console + local feedback sessions + runtime verification
...
- 기본적으로 앱을 마음대로 조작하지 않음. Navigation은 debug-only
...
MCP server는 `fixthis mcp`라는 desktop process로 실행한다. Android debug app에는 sidekick만 들어간다.
...
- feedback session state와 `.fixthis/feedback-sessions/` persistence는 MCP
  process가 소유
- 보안/프라이버시 부담 감소

---

## 17. 결정 16: MCP tool은 agent workflow 단위로 묶는다

### 결정

v1 MCP는 low-level automation primitive를 많이 노출하지 않고, agent가
사용하는 workflow 단위 tool을 제공한다.

Feedback console workflow tools:
...
```text
open_feedback_console
list_feedback/read_feedback
capture_screen/navigate_app
get_current_screen
verify_ui_change
status
```
...
새 workflow의 핵심은 `fixthis_open_feedback_console`와 feedback queue
tools다.
...
→ fixthis_read_feedback이 agent-readable context 반환
```

Current implementation note: an earlier plan kept `fixthis_get_ui_feedback` as
a single-item compatibility wrapper, but the current mainline MCP tool surface
no longer includes it. Feedback capture goes through
`fixthis_open_feedback_console`, Save, Send, and `fixthis_read_feedback`.
Historical references to the wrapper in this decision log describe an earlier
product shape.

### 범위 제한

`fixthis_navigate_app`은 `back`, `tap`, `swipe`만 지원한다. Arbitrary
typing, scripted automation, network inspection, HTTP/WebSocket mocking,
cloud sync는 FixThis MCP 범위가 아니다.
...
- `--package` 또는 `.fixthis/project.json`에서 applicationId 확인
- device/emulator 확인
- MCP client config JSON 출력
- local feedback console 실행
- repo sample debug app build/install/launch
- sidekick 연결 확인
- 문제 진단
...
가장 권장하는 설치 방식은 Gradle plugin이다. 아래 coordinate는 artifact가 publish된 뒤의 외부 설치 예시다.
...
| CLI setup JSON only | 프로젝트 수정 없음 | Gradle 설정/IDE 연동 자동화 없음 |

### 결론

Primary after artifacts are published:
...
FixThis는 selected node를 정확히 찾지 못해도 annotation을 생성해야 한다.
...
FixThis는 screenshot과 UI text를 다룬다. 민감 정보가 포함될 수 있다.
...
FixThis는 DTA, mobile-mcp, appium-mcp와 같은 범용 모바일 자동화 플랫폼이 아니다.
...
FixThis가 같은 방향으로 가면 차별화가 약해지고 scope가 커진다.
...
FixThis MCP는 기본적으로 file write/code edit tool을 제공하지 않는다.

### 이유

Codex, Claude Code, Cursor 같은 AI coding agent는 이미 repository file edit 기능을 갖고 있다. FixThis가 코드 쓰기까지 맡으면 역할이 불필요하게 커지고 권한 문제가 생긴다.
...
```text
FixThis tells AI what and where.
...
FixThis는 MCP-only 제품이 아니다. MCP-enabled 제품이다.
...
앱 안 annotation 흐름은 더 쉽다는 가정이 있었다.
...
Historical conclusion: 제품 첫 경험은 앱 안 annotation이어야 한다고 보았다.
Current mainline correction: 현재 shipped flow는 MCP feedback console first다.
...
최초 공개 버전부터 `FixThisRoot` wrapper 기반 MVP로 가지 않고, autoinit/overlay 방향으로 간다.

### 이유

나중에 UX를 바꾸면 사용자 API와 문서가 크게 바뀐다. 처음부터 최종형 UX를 잡는 것이 좋다.

초기 최소 구현의 사용자 경험은 다음으로 보았다.
...
첫 공개 목표는 v1 Full로 잡았다. 현재 mainline v1은 source index, CLI,
MCP feedback console, local sessions, stable target evidence를 포함하고,
앱 안 annotation overlay는 포함하지 않는다.
...
7. MCP server와 agent workflow tools
...
Select UI
→ 사용자가 target을 탭
→ FixThis가 best semantics candidate를 highlight
→ comment sheet 표시
...
FixThis는 빠른 기본 선택과 수동 보정 능력을 모두 갖는다.
...
현재 `sample/`의 XML/ViewBinding/AppCompat starter app은 FixThis 검증에 적합하지 않다. v1 구현에서는 `sample/`을 Jetpack Compose 전용 테스트 앱으로 교체한다.
...
FixThis의 핵심은 Compose semantics, RootForTest, screenshot crop, source candidates, Smart Select를 검증하는 것이다. XML/View sample은 이 목적을 검증하지 못한다.
...
```text
Source candidates are best-effort. FixThis provides search hints and likely files, but does not guarantee exact source line mapping for every UI node.
```
...
Android app-only export에서는 screenshot path가 Android app sandbox 내부 path일 수 있다. CLI/MCP 경로에서는 desktop AI agent가 읽을 수 있도록 screenshot artifact를 `.fixthis/artifacts/` 아래로 pull하거나 resource로 제공한다.
...
{
  "screenshot": {
    "fullPath": "/data/user/0/.../cache/fixthis/full.png",
    "cropPath": "/data/user/0/.../cache/fixthis/crop.png",
    "desktopFullPath": ".fixthis/artifacts/annotation-1/annotation-1-full.png",
    "desktopCropPath": ".fixthis/artifacts/annotation-1/annotation-1-crop.png"
  }
}
```

구현된 CLI/MCP 경로는 Android screenshot 파일을 직접 desktop path로 노출하지 않고, sidekick bridge의 `readScreenshot` method로 현재 annotation의 `full` 또는 `crop` PNG를 읽은 뒤 `.fixthis/artifacts/<annotation-id>/` 아래에 저장한다. Bridge는 명시적 path 읽기를 허용하지 않고 현재 annotation screenshot만 읽는다.
...
FixThis for Android Compose는 다음 원칙으로 만든다.
...
zero-code install
MCP feedback console first
browser-side target selection plus visual-area fallback
debug app status indicator only
no required testTags
no AccessibilityService
...
nullable targetEvidence for stable handoff
local-first export
