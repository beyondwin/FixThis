...
2. **고급 UX:** MCP 연결 → AI agent가 실행 중인 앱 화면 context를 직접 조회하고 수정 후 검증.
...
8. MCP 연결 시 사용자가 앱에서 선택한 UI를 AI agent가 바로 받을 수 있다.
...
### 7.2 MCP 시나리오: AI agent가 선택 요청

```text
1. 사용자가 AI agent에게 말한다.
   "PointPatch로 수정할 UI를 선택하게 해줘."

2. AI agent가 MCP tool `pointpatch_get_ui_feedback`을 호출한다.

3. 앱에 selection overlay가 켜진다.

4. 사용자가 앱에서 수정할 UI를 탭한다.

5. 사용자가 앱 안 comment sheet에서 원하는 변경사항을 입력한다.

6. MCP tool result로 selectedNode, nearbyNodes, screenshot crop, sourceCandidates가 반환된다.

7. AI agent가 source candidate를 참고해 코드 수정한다.

8. AI agent가 `pointpatch_verify_ui_change`로 변경 반영 여부를 확인한다.
```
...
- Markdown
- JSON
- AI client-specific prompt template

AI client-specific templates:

- Codex
- Claude Code
- Cursor
- generic MCP-aware agent
