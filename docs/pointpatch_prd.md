...
2. **고급 UX:** MCP feedback console → desktop browser에서 device 선택,
   screenshot capture, Select/Navigate mode, draft feedback 작성, agent-readable
   handoff batch 생성.
...
8. MCP feedback console에서 사용자가 component 또는 custom area를 선택해
   draft feedback을 만들고, sent handoff batch로 agent에게 전달할 수 있다.
...
2. browser feedback console에서 연결된 ADB device를 선택한다.

3. Capture로 현재 Android 화면을 session snapshot으로 저장한다.

4. Select mode에서 component를 클릭하거나 custom area를 drag한다.

5. 사용자가 comment를 입력하고 Add Item으로 draft feedback을 만든다.

6. Send Draft to Agent를 누르면 console은 외부 AI API를 호출하지 않고,
   `.pointpatch/feedback-sessions/` 아래에 sent handoff batch를 기록한다.

7. AI agent가 `pointpatch_read_feedback`으로 batch, item, screen,
   selected node, source candidate, screenshot path를 읽는다.

8. AI agent가 source candidate를 참고해 코드 수정하고,
...
- browser에서 Select/Navigate mode, draft, Sent History workflow 제공
```
...
```text
device picker에서 active ADB device 선택
→ Capture
→ Select mode에서 component click 또는 custom drag area 선택
→ comment 입력
→ Add Item으로 draft 생성
→ Send Draft to Agent로 persisted handoff batch 생성
→ Sent History와 pointpatch_read_feedback에서 batch 확인
```

Navigate mode는 debug-only `back`, `tap`, `swipe` one-step action만 수행한다.
...
- Select mode에서 component click 또는 custom drag area로 draft feedback을 만들 수 있다.
- Send Draft to Agent 이후 `pointpatch_read_feedback`이 sent handoff batch를 반환한다.
