...
8. MCP feedback console에서 사용자가 live preview를 탐색하고, Add로 freeze한
   화면에 component 또는 custom area feedback을 여러 개 쌓은 뒤, Save 한 번으로
   evidence snapshot과 item들을 저장하고 Send로 handoff batch를 만들 수 있다.
...
5. component를 클릭하거나 custom area를 drag하고 comment를 입력한다.
   같은 frozen preview에 여러 pending item을 추가할 수 있다.
...
- browser에서 live preview navigation, Add freeze, Save evidence batch,
  Send handoff workflow 제공
```
...
→ comment 입력 후 여러 pending item 추가
